...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const slideNames = ['Expertise Judiciaire', 'Évaluation Vénale', 'Évaluation Bancaire & Garantie Hypothécaire', 'Expertise Patrimoniale', 'Évaluation Locative'];
    const totalSlides = 5;
    let currentIndex = 0;
    let startX = 0;
    let isDragging = false;
    ...
    function updateCarousel(index) {
        ...
        setTimeout(() => {
            label.textContent = slideNames[currentIndex];
...
(function() {
    ...
    var cityNames = Object.keys(MADAGASCAR_LOCATIONS).filter(function(k) {
        return k !== 'Autre';
    }).sort(function(a, b) {
    ...
    var cityAc = initAutocomplete('city-input', 'city', 'city-list', cityNames, {
        ...
        onSelect: function(selectedCity) {
            ...
            if (selectedCity === 'Autre') {
                ...
                qInput.placeholder = "Sélectionnez d'abord une ville...";
                qInput.value = '';
                ...
                var quartiers = (MADAGASCAR_LOCATIONS[selectedCity] || []).slice().sort(function(a, b) {
                    return a.localeCompare(b);
                });
                qInput.disabled = false;
                qInput.placeholder = 'Rechercher un quartier...';
                if (quartierAc) {
    ...
    qInputDef.placeholder = 'Rechercher un quartier...';
    quartierAc.setItems((MADAGASCAR_LOCATIONS['Antananarivo'] || []).slice().sort(function(a, b) {
        return a.localeCompare(b);
    }));
    document.getElementById('city-input').addEventListener('input', function(e) {
        ...
        if (val !== '' && !MADAGASCAR_LOCATIONS[val]) {
            qInput.disabled = false;
            qInput.placeholder = 'Veuillez taper le quartier...';
...
Une nouvelle demande d'expertise a été soumise.Voici les détails:---IDENTIFICATION---
Nom complet:${name}
Email:${email}
...
Demande générée depuis le site ExpertImmo.mg`;
            ...
            if (res.ok) {
                ...
                btn.style.opacity = '1';
                if (successBox) {
                    successBox.textContent = `✓ Votre demande a bien été envoyée.Le Cabinet vous recontactera sous 24h ouvrées à l'adresse ${email}.`;
            ...
            btn.textContent = '✗ Erreur — voir message ci-dessous';
            btn.style.background = '#c0392b';
            btn.style.opacity = '1';
            if (fallback) {
                if (err.message === 'Erreur serveur' || err.message === 'Failed to fetch') {
                    fallback.textContent = 'Test local détecté : PHP n\'est pas exécuté. (Ou erreur réseau). Testez sur le serveur réel !';
                } else {
        ...
        if (entry.isIntersecting) {
            ...
            el.dataset.timer = setInterval(() => {
                ...
                if (current >= target) {
                    ...
                    el.innerText = prefix + formatNumber(target) + suffix;
                } else {
